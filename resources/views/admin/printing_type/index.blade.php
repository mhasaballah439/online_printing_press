            ...
            <div class="content-body">
                <section id="file-export">
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                @if(admin()->check_route_permission('admin.printing_type.index') == 1)
                                <div class="card-header" style="padding-bottom: 0px">
                                    <h4 class="card-title">{{__('msg.printing_type')}}</h4>
                                    <a class="heading-elements-toggle"><i
                                            class="la la-ellipsis-v font-medium-3"></i></a>
                                    <div class="heading-elements">
                                        <ul class="list-inline mb-0">
                                            @if(admin()->check_route_permission('admin.printing_type.create') == 1)
                                                <li>
                                                    <a href="{{route('admin.printing_type.create')}}" style="border-radius: 10px;padding: 10px"
                                                ...
                                                @foreach($printing_type as $type)
                                                    <tr class="row_{{$type->id}}">
                                                        ...
                                                        <td class="d-flex align-items-center justify-content-sm-center">
                                                            @if(admin()->check_route_permission('admin.printing_type.delete') == 1)
                                                            <a href="#" item_id="{{$type->id}}" class="delete_item btn btn-danger">
                                                                <i class="la la-trash"></i></a>
                                                            @endif
                                                                @if(admin()->check_route_permission('admin.printing_type.edit') == 1)
                                                            <a href="{{route('admin.printing_type.edit',$type-> id)}}"
    ...
    <script>
        $(document).on('click','.delete_item',function (){
            ...
            $(document).on('click','.confirm_delete',function (){
                $.ajax({
                    type : 'post',
                    url: "{{route('admin.printing_type.delete')}}",
                    data:{
                        '_token' : "{{csrf_token()}}",
                        'id' : item_id
                    },
